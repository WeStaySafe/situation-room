defmodule SituationRoom.EventIndexer do
  ...
  def index() do
    ...
    Enum.map(network_informations, fn network_information ->
      ...
      Enum.map(network_information.contract_monitors, fn contract_monitor ->
        ...
        {:ok, logs} = get_logs_from_contract(network_information.rpc_endpoint, contract_monitor, previous_block_number, latest_block_number)
          if(not is_nil(logs) and not Enum.empty?(logs)) do
            Enum.map(logs, fn log ->
              IO.inspect(log)
              # TODO: save events (still need to add context/model)
            end)
          end
      end)
    end)
  ...
  defp get_logs_from_contract(rpc_endpoint, contract_monitor, previous_block_number, latest_block_number) do
    payload = %{
      "jsonrpc" => "2.0",
